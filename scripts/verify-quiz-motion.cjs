...
(async () => {
  ...
  try {
    for (const width of [390,1440]) {
      ...
      await page.locator('#dialog-close').click();
      const reviews = page.locator('.reviews-carousel');
      const reviewViewport = page.locator('.reviews-viewport');
      const reviewTrack = page.locator('.reviews-track');
      await reviewViewport.scrollIntoViewIfNeeded();
      await page.waitForFunction(() => document.querySelector('.reviews-carousel').dataset.offscreen === 'false');
      assert.equal(await reviews.locator('.reviews-group').first().locator('.review-quote').count(), 6, 'six distinct client reviews');
      assert.equal(await reviews.locator('.reviews-group[aria-hidden=true][inert]').count(), 1, 'loop copy is hidden from assistive technology');
      const reviewFit = await reviews.locator('.reviews-group').first().locator('.review-quote').evaluateAll(cards => cards.every(card => card.scrollWidth <= card.clientWidth && card.scrollHeight <= card.clientHeight && card.getBoundingClientRect().width < document.querySelector('.reviews-viewport').clientWidth));
      assert.ok(reviewFit, 'each complete review fits the carousel without cropped text');
      const reviewBefore = await reviewTrack.evaluate(el => getComputedStyle(el).transform);
      await page.waitForTimeout(700);
      assert.notEqual(await reviewTrack.evaluate(el => getComputedStyle(el).transform), reviewBefore, 'reviews move continuously');
      await reviewViewport.hover();
      assert.equal(await reviewTrack.evaluate(el => getComputedStyle(el).animationPlayState), 'running');
      await page.locator('#google-reviews').screenshot({ path: '.cache/quiz-reviews-' + width + '.png' });
      await reviewTrack.evaluate(el => { el.getAnimations()[0].currentTime = 109950; });
      const reviewJoinBefore = await reviews.locator('.reviews-group').nth(1).evaluate(el => el.getBoundingClientRect().left);
      await page.waitForTimeout(150);
      const reviewJoinAfter = await reviews.locator('.reviews-group').first().evaluate(el => el.getBoundingClientRect().left);
      assert.ok(Math.abs(reviewJoinAfter - reviewJoinBefore) < 18, 'review loop has a seamless join');
      await page.locator('[data-start]').first().click();
      assert.equal(await page.locator('#quiz').isVisible(), true);
      await page.waitForFunction(() => document.querySelector('.reviews-carousel').dataset.offscreen === 'true');
      assert.equal(await reviewTrack.evaluate(el => getComputedStyle(el).animationPlayState), 'paused', 'hidden reviews do not keep animating');
      ...
      assert.equal(await reviewTrack.evaluate(el => getComputedStyle(el).animationName), 'none');
      assert.equal(await reviewViewport.evaluate(el => getComputedStyle(el).overflowX), 'auto');
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
      report.checks.push({ width, continuousStrip: 'passed', fullPhotos: 'passed', seamlessLoop: 'passed', noPauseControls: 'passed', hoverKeepsMoving: 'passed', tattooRain: 'passed', smallSymbols: 'passed', reducedMotion: 'passed', modal: 'passed', quiz: 'passed', sixReviews: 'passed', reviewMotion: 'passed', reviewTextFit: 'passed', reviewSeamlessLoop: 'passed' });
      await context.close();
    }
